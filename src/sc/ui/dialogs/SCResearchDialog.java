...
public class SCResearchDialog extends BaseDialog {
    ...
    public SCResearchDialog() {
        ...
        }).visible(() -> showTechSelect = TechTree.roots
                .count(node -> !(node.requiresUnlock && !node.content.unlockedHost())) > 1).minWidth(300f);
        ...
        shown(() -> {
            ...
            Planet currPlanet = ui.planet.isShown() ? ui.planet.state.planet
        ...
        SCPlanetDialog planet = new SCPlanetDialog();
        hidden(planet::setup);
    ...
    public void rebuildItems() {
        items = new ItemSeq() {
...

            {
                ...
                Planet rootPlanet = lastNode.planet != null ? lastNode.planet
                        : content.planets().find(p -> p.techTree == lastNode);
            ...
            public void add(Item item, int amount) {
                // only have custom removal logic for when the sequence gets items taken out of
                // it (e.g. research)
                if (amount < 0) {
    ...
    void treeLayout() {
        ...
        LayoutNode[] rightHalf = Arrays.copyOfRange(node.children, Mathf.ceil(node.children.length / 2f),
    ...
    void checkNodes(TechTreeNode node) {
        ...
        if (!locked && (node.parent == null || node.parent.visible))
            node.visible = true;
    ...
    boolean selectable(TechNode node) {
        // there's a desync here as far as sectors go, since the client doesn't know
        // about that, but I'm not too concerned
        ...
        public void rebuildAll() {
            ...
            for (TechTreeNode node : nodes) {
                ...
                button.update(() -> {
                    ...
                    button.getStyle().up = !locked(node.node) ? Tex.buttonOver
                            : !selectable(node.node) || (!canSpend(node.node) && !net.client()) ? Tex.buttonRed
                                    : Tex.button;

                    ((TextureRegionDrawable) button.getStyle().imageUp)
                            .setRegion(node.selectable ? node.node.content.uiIcon : Icon.lock.getRegion());
                    button.getImage()
                            .setColor(!locked(node.node) ? Color.white : node.selectable ? Color.gray : Pal.gray);
        ...
        boolean canSpend(TechNode node) {
            ...
            for (int i = 0; i < node.requirements.length; i++) {
                if (node.finishedRequirements[i].amount < node.requirements[i].amount
                        && items.has(node.requirements[i].item)) {
                    return true;
                }
            }
        ...
        void rebuild(@Nullable boolean[] shine) {
            ...
            infoTable.addListener(new InputListener() {
                ...
                public void exit(InputEvent event, float x, float y, int pointer, Element fromActor) {
                    ...
                    if (hoverNode == button
                            && !(e != null && (e.isDescendantOf(infoTable) || e.isDescendantOf(hoverNode)))
                            && (Core.app.isDesktop() || pointer == 0)) {
                        hoverNode = null;
                        rebuild();
                    }
                }
            ...
            infoTable.update(() -> infoTable.setPosition(button.x + button.getWidth(), button.y + button.getHeight(),
                    Align.topLeft));
                            ...
                            desc.table(t -> {
                                ...
                                if (selectable) {
                                    ...
                                    if (Structs.contains(node.finishedRequirements, s -> s.amount > 0)) {
                                        ...
                                        for (int i = 0; i < node.requirements.length; i++) {
                                            ...
                                            used += node.finishedRequirements[i].item.cost
                                                    * node.finishedRequirements[i].amount;
                                            if (shine != null)
                                                shiny |= shine[i];
                                        }

                                        Label label = t.add(Core.bundle.format("research.progress",
                                                Math.min((int) (used / sum * 100), 99))).left().get();
                                    ...
                                    for (int i = 0; i < node.requirements.length; i++) {
                                        ...
                                        t.table(list -> {
                                            int reqAmount = debugShowRequirements ? req.amount
                                    ...
                                    t.table(r -> {
                                        ...
                                        for (Objective o : node.objectives) {
                                            ...
                                            r.image(o.complete() ? Icon.ok : Icon.cancel,
                                                    o.complete() ? Color.lightGray : Color.scarlet).padLeft(3);
                                            r.row();
                                        }
                                    });
                                    t.row();
            ...
            if (node.content.description != null && node.content.inlineDescription && selectable) {
                infoTable.table(t -> t.margin(3f).left().labelWrap(node.content.displayDescription())
        ...
        public void drawChildren() {
            ...
            for (TechTreeNode node : nodes) {
                ...
                for (TechTreeNode child : node.children) {
                    ...
                    if (Mathf.equal(Math.abs(node.y - child.y), Math.abs(node.x - child.x), 1f)
                            && Mathf.dstm(node.x, node.y, child.x, child.y) <= node.width * 3) {
