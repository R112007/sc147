    ...
    protected void generate() {
        ...
        class Room {
            ...
            void join(int x1, int y1, int x2, int y2) {
                ...
                brush(pathfind(x1, y1, x2, y2,
                        tile -> (tile.solid() ? 50f : 0f) + noise(tile.x, tile.y, 2, 0.4f, 1f / nscl) * 500,
            ...
            void connect(Room to) {
                ...
                if (alt) {
                    midpoint.add(Tmp.v2.set(1, 0f)
                            .setAngle(Angles.angle(to.x, to.y, x, y) + 90f * (rand.chance(0.5) ? 1f : -1f))
                            .scl(Tmp.v1.dst(x, y) * 2f));
                } else {
                        ...
                        for (int y = -rad; y <= rad; y++) {
                            ...
                            if (Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)) {
                                ...
                                if (Mathf.within(x, y, rad - 1) && !other.floor().isLiquid) {
                                    ...
                                    other.setFloor(
                                            (Floor) (floor == Blocks.sand || floor == Blocks.salt ? Blocks.sandWater
        ...
        pass((x, y) -> {
            ...
            if (value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)) {
                ...
                if (!(floor == Blocks.ice || floor == Blocks.iceSnow || floor == Blocks.snow
                        || floor.asFloor().isLiquid)) {
                    floor = spore ? (deep ? Blocks.taintedWater : Blocks.darksandTaintedWater)
                            : (deep ? Blocks.water
                                    : (floor == Blocks.sand || floor == Blocks.salt ? Blocks.sandWater
        ...
        Seq<Block> ores = Seq.with(SCOre.oreLv, SCOre.oreLi);
        ...
        if (Simplex.noise3d(seed, 2, 0.5, scl, sector.tile.v.x, sector.tile.v.y, sector.tile.v.z) * nmag + poles > 0.25f
                * addscl) {
            ores.add(SCOre.oreTandanzhi);
        }

        if (Simplex.noise3d(seed, 2, 0.5, scl, sector.tile.v.x + 1, sector.tile.v.y, sector.tile.v.z) * nmag
                + poles > 0.5f * addscl) {
            ores.add(SCOre.oreXi);
        }
        ...
        pass((x, y) -> {
            ...
            if (floor == Blocks.hotrock) {
                ...
                if (noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))) {
                    if (noise > 0.72f) {
                        floor = noise > 0.78f ? Blocks.taintedWater
                                : (floor == Blocks.sand ? Blocks.sandWater : Blocks.darksandTaintedWater);
                    } else {
        ...
        if (sector.hasEnemyBase()) {
            basegen.generate(tiles, enemies.map(r -> tiles.getn(r.x, r.y)), tiles.get(spawn.x, spawn.y),
