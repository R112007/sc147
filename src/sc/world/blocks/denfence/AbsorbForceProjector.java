...
import arc.util.*;
import arc.util.io.*;
import sc.annotations.Annotations.*;
...
public class AbsorbForceProjector extends Block {
    ...
    protected static AbsorbForceBuild paramEntity;
    protected static Effect paramEffect;
    protected static final Cons<Bullet> shieldConsumer = bullet -> {
        if (bullet.team != paramEntity.team &&
                Intersector.isInRegularPolygon(((AbsorbForceProjector) (paramEntity.block)).sides, paramEntity.x,
                        paramEntity.y, paramEntity.realRadius(),
                        ((AbsorbForceProjector) (paramEntity.block)).shieldRotation, bullet.x, bullet.y)) {
            bullet.absorb();
    ...
    public AbsorbForceProjector(String name) {
        super(name);
        update = true;
        solid = true;
    ...
    public void setBars() {
        super.setBars();
        addBar("shield", (AbsorbForceBuild entity) -> new Bar("stat.shieldhealth", Pal.accent,
    ...
    public void setStats() {
        ...
        if (consItems && itemConsumer instanceof ConsumeItems coni) {
            ...
            stats.add(Stat.booster, StatValues.itemBoosters("+{0} " + StatUnit.shieldHealth.localized(),
                    stats.timePeriod, phaseShieldBoost, phaseRadiusBoost, coni.items));
            stats.add(Stat.booster,
                    StatValues.speedBoosters("", coolantConsumption, Float.MAX_VALUE, true, this::consumesLiquid));
        }
    }
    ...
    public class AbsorbForceBuild extends Building implements Ranged, ExplosionShield {
        ...
        public boolean absorbExplosion(float ex, float ey, float damage) {
            boolean absorb = !broken
                    && Intersector.isInRegularPolygon(sides, x, y, realRadius(), shieldRotation, ex, ey);
            if (absorb) {
        ...
        public void overwrote(Seq<Building> previous) {
            if (previous.size > 0 && previous.first().block == block
                    && previous.first() instanceof AbsorbForceBuild b) {
                broken = b.broken;
                buildup = b.buildup;
            }
        }
